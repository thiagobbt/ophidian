...
void method_of_means_and_medians::build_topology(const method_of_means_and_medians::point clock_source, const std::vector<method_of_means_and_medians::point> &flip_flop_positions, clock_tree_synthesis::clock_topology &clock_topology, clock_tree_synthesis::embedding &embedding)
{
...
void method_of_means_and_medians::build_topology(const method_of_means_and_medians::point clock_source, const std::vector<method_of_means_and_medians::point> &flip_flop_positions, clock_tree_synthesis::clock_topology &clock_topology)
{
    clock_tree_synthesis::embedding embedding(clock_topology.graph());
    std::vector<method_of_means_and_medians::point> flip_flop_positions_copy = flip_flop_positions;
    auto source_node = clock_topology.node_create();
    embedding.node_position(source_node, clock_source);
    build_topology(source_node, flip_flop_positions_copy.begin(), flip_flop_positions_copy.end(), clock_topology, embedding, true);
...
void method_of_means_and_medians::build_topology(clock_topology::node parent_node, std::vector<point>::iterator positions_begin, std::vector<point>::iterator positions_end, clock_tree_synthesis::clock_topology &clock_topology, clock_tree_synthesis::embedding & embedding, bool sort_by_x_coordinate)
{
    point center_of_mass = calculate_center_of_mass(positions_begin, positions_end);
    auto center_of_mass_node = clock_topology.node_create();
    ...
    if (size > 1) {
        ...
        build_topology(center_of_mass_node, positions_begin, positions_middle, clock_topology, embedding, !sort_by_x_coordinate);
        build_topology(center_of_mass_node, positions_middle, positions_end, clock_topology, embedding, !sort_by_x_coordinate);
    }
}
}
}
