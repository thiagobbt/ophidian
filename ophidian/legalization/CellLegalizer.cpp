...
CellLegalizer::CellLegalizer(design::Design & design) : mDesign(design), mFenceRegionIsolation(design)
{
}
...
void CellLegalizer::buildTree(const std::vector<circuit::Cell> legalizedCells) {
...
bool CellLegalizer::legalizeCell(const circuit::Cell & targetCell, const geometry::Point & targetPosition, const Box & legalizationRegion)
{
    ...
    while (overlaps.size() > 0) {
        ...
        if (moveLeft) {
            if ((min_corner_i.x() - width_j) < legalizationRegion.min_corner().x()) {
                if (!hasFence) mFenceRegionIsolation.restoreAllFenceCells();
                return false;
            } else {
                target_j = Box(
                    Point(min_corner_i.x() - width_j, min_corner_j.y()),
