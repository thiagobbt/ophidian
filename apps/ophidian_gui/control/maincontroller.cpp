...
void MainController::buildICCAD2017(std::string cells_lef, std::string tech_lef, std::string placed_def)
{
    ...
    /* Building */
    mBuilder = new ophidian::designBuilder::ICCAD2017ContestDesignBuilder(cells_lef, tech_lef, placed_def);
    init();
}
...
void MainController::buildICCAD2015(std::string lef, std::string def, std::string verilog)
{
    ...
    /* Building */
    mBuilder = new ophidian::designBuilder::ICCAD2015ContestDesignBuilder(lef, def, verilog);
    init();
}
...
void MainController::createQuads()
{
    ...
    for (auto cellIt = mDesign->netlist().begin(ophidian::circuit::Cell()); cellIt != mDesign->netlist().end(ophidian::circuit::Cell()); cellIt++)
    {
        ...
        ophidian::util::MultiBox cellGeometry = mDesign->placementMapping().geometry(*cellIt);
...
void MainController::update(Quad quad)
{
    ...
    mDesign->placement().placeCell(first.mCell, ophidian::util::Location(newOrigin.position.x, newOrigin.position.y));

    ophidian::util::MultiBox cellGeometry = mDesign->placementMapping().geometry(first.mCell);
...
ophidian::geometry::Point MainController::cellSize(const ophidian::circuit::Cell & cell)
{
    ...
    ophidian::util::MultiBox cellGeometry = mDesign->placementMapping().geometry(cell);
    for (auto cellBoxIt = cellGeometry.begin(); cellBoxIt != cellGeometry.end(); cellBoxIt++)
    {
