...
TEST_CASE_METHOD(Def2PlacementFixture, "Def2Placement: Test if def2placement mapped cells's position correctly according .def file.", "[placement][Def]")
{
    ...
    REQUIRE(placement.cellLocation(netlist.find(circuit::Cell(), "u1")) == util::Location(3420, 6840));
    REQUIRE(placement.cellLocation(netlist.find(circuit::Cell(), "u2")) == util::Location(3420, 3420));
    REQUIRE(placement.cellLocation(netlist.find(circuit::Cell(), "u3")) == util::Location(6840, 3420));
    REQUIRE(placement.cellLocation(netlist.find(circuit::Cell(), "u4")) == util::Location(12160, 6840));
    REQUIRE(placement.cellLocation(netlist.find(circuit::Cell(), "lcb1")) == util::Location(0, 10260));
}
